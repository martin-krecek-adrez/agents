...
| Azure budget spike | Outlook/Gmail Azure budget alerts | Review cost source before it burns the monthly budget | July budget reset triggered repeatedly early: 26.19 USD on 2026-07-03 02:33 UTC, 50.03 USD on 2026-07-06 00:41 UTC, and 76.27 USD against 100 USD monthly budget on 2026-07-08 14:36 UTC. | 2026-07-10 |
...
| dbt Mews rate-pricing freshness | Outlook dbt Cloud / Martin correction | Check only if warning repeats again or blocks reporting | 2026-07-05 Production daily run warned again on the same 4 per-property Mews rate-pricing freshness tests, each WARN 17. Martin previously confirmed this pattern was OK, so treat as a watch unless it repeats or coincides with missing reporting data. | 2026-07-06 |
...
| Booking scraping empty snapshot window | Teams Data pipeline / data-factory task note | Resolved unless next default run fails on Booking scraping row-count validation | 2026-07-03 and 2026-07-04 pipeline-orchestrator default runs failed because `configs/scraping_booking.yaml` refreshed a 2-day window with no Booking scraping files. `data-factory` removed fatal `min_rows` checks for Booking rooms/prices; remaining follow-up is warning-level freshness or row-count reporting outside the fatal path. | 2026-07-06 |
...
| Snowflake Trust Center network policy violation | Outlook Snowflake Trust Center | Check whether this is accepted posture or needs scoped remediation | 2026-07-13 Outlook Snowflake email again reports 1 critical CIS violation: users not covered by network policy to restrict access to trusted IPs. This is separate from the resolved scanner-charge case. | 2026-07-14 |
| Snowflake budget trendline | Outlook Snowflake notification | Review only if spend keeps trending above plan | 2026-07-02 Snowflake notification says account `VB12611` budget `SNOWFLAKE.LOCAL.ACCOUNT_ROOT_BUDGET` is above its linear trendline; invoice and usage statement also arrived overnight. | 2026-07-03 |
| Power BI Usage Metrics refresh | Outlook Power BI | Resume only if Usage Metrics is still useful for PROD workspace monitoring | 2026-07-14 Outlook Power BI email says scheduled refresh was paused for `Usage Metrics Report` semantic model in `Adrez BI - PROD` because no one viewed dependent dashboards/reports for two months. | 2026-07-14 |
