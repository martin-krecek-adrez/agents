...
| Waiting | Booking SRS timeline consumer deploy | Linear DTE-90 / Hapl / dbt task notes | DTE-90 is assigned to Martin Hapl and crosses an external unowned consumer checkout. Do not surface it as Martin Křeček execution work or inspect the consumer repository; handle only Snowflake cleanup if explicitly reopened with a concrete owned scope. | 2026-07-20 |
| Open | Snowflake critical service key rotation | Linear DTE-116 | Linear tracks the work with due date 2026-10-25. Identify affected service users, rotate required keys, and validate dependent services; do not rank as urgent without a fresh security trigger. | 2026-07-24 |
| Open | Market Overview laptop-state handoff | Linear DTE-145 / DTE-160 / reporting PR #63 | Audit is complete. Before cutover, Martin must decide how to handle the stale `peer_tail_pace_summary` input, preserve the genuine unpushed docs, complete the environment contract, migrate the identified non-rebuildable state, and keep the eight legacy standalone HTML dashboards out of the port. The 2026-07-26 shadow guard is also blocked by upstream drift in `rates_calendar.doc.js` and `rates_calendar.js`. | 2026-07-27 |
